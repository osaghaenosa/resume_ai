...
const NavLink: React.FC<{ to: string; children: React.ReactNode; onClick?: () => void }> = ({ to, children, onClick }) => {
  ...
  return (
    <a href={to} onClick={handleClick} className={`${isActive ? 'text-white' : 'text-gray-300'} hover:text-white transition-colors duration-300 px-4 py-3 rounded-md text-base font-medium block relative group`}>
      {children}
...
export default function Header({ isApp = false, onUpgradeClick }: HeaderProps) {
  ...
  const AppNavButton: React.FC<{ to: string; children: React.ReactNode; isMobile?: boolean }> = ({ to, children, isMobile }) => {
    ...
    return (
      <button 
        ...
        className={`${isMobile ? 
          `w-full text-left block px-4 py-3 rounded-md text-base font-medium ${isActive ? 'text-white bg-gray-800' : 'text-gray-300'} hover:text-white hover:bg-gray-800 transition-colors duration-300` : 
          `text-gray-300 hover:text-white transition-colors duration-300 px-3 py-2 rounded-md text-sm font-medium relative group ${isActive ? 'text-white' : ''}`
        }`}
      >
        {children}
        {!isMobile && (
    ...
    <header className={`sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-gray-900 bg-opacity-95 backdrop-blur-md shadow-xl' : 'bg-transparent'}`}>
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center">
            <div className="ml-10 flex items-baseline space-x-2">
              {isApp ? (
                appNavLinks.map(link => <AppNavButton key={link.to} to={link.to}>{link.text}</AppNavButton>)
              ) : (
            ...
            <div className="ml-6 flex items-center">
              {currentUser ? (
                  ...
                  <div className="relative" ref={userMenuRef}>
                    ...
                    {isUserMenuOpen && isApp && (
                      <div className="absolute right-0 mt-2 w-48 bg-gray-800 rounded-md shadow-lg py-1 ring-1 ring-black ring-opacity-5 z-50 border border-gray-700">
                        <button onClick={() => handleNavigation('/profile')} className="block w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">Profile</button>
                        <button onClick={() => handleNavigation('/settings')} className="block w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">Settings</button>
                        <div className="border-t border-gray-700 my-1"></div>
                        <button onClick={handleLogout} className="block w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-gray-700 hover:text-white transition-colors">Logout</button>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-3 ml-4">
                  <button onClick={handleLogin} className="text-gray-300 hover:text-white transition-colors duration-300 px-3 py-2 rounded-md text-sm font-medium">Login</button>
                  <button onClick={handleSignup} className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-2 px-4 rounded-md text-sm hover:from-cyan-600 hover:to-blue-700 transition-all duration-300 shadow-md">Sign Up</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="md:hidden flex items-center">
            ...
            {!isApp && !currentUser && (
              <div className="flex items-center space-x-2 mr-3">
                ...
                <button onClick={handleSignup} className="bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold py-1.5 px-3 rounded text-xs hover:from-cyan-600 hover:to-blue-700 transition-all duration-300 shadow-md">
                  Sign Up
                </button>
              </div>
            )}
      ...
      <div className={`md:hidden bg-gray-900 shadow-xl border-t border-gray-800 transition-all duration-300 ease-in-out ${isMenuOpen ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
        <div className="px-2 pt-2 pb-3 space-y-1">
          {isApp ? (
            appNavLinks.map(link => <AppNavButton key={link.to} to={link.to} isMobile>{link.text}</AppNavButton>)
          ) : (
            landingNavLinks.map(link => (
              <NavLink key={link.to} to={link.to} onClick={() => setIsMenuOpen(false)}>
                {link.text}
          ...
          {currentUser ? (
            <div className="mt-4 pt-4 border-t border-gray-800 px-2 space-y-2">
              ...
              <AppNavButton to="/profile" isMobile>Profile</AppNavButton>
              <AppNavButton to="/settings" isMobile>Settings</AppNavButton>
              <button 
